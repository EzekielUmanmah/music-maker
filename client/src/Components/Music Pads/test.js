...
const Volume = () => {
  ...
  const handleVolume = (val) => {
    ...
    if(mute){setState(state => ({...state, mute: !mute}))};
    (currentVol === 0) ? setVolIcon('fas fa-volume-mute'):
    (currentVol > vol) ? setVolIcon('fas fa-volume-up'): 
    (currentVol < vol) ? setVolIcon('fas fa-volume-down'):null;
    refsArray.current.map(x => {
        x.volume = currentVol / 100;
        setState(state => ({...state, str: "Volume "+ currentVol}));
      });
    val.target.style.background = 'linear-gradient(to right, #82CFD0 0%, #82CFD0 '+currentVol +'%, #fff ' + currentVol + '%, white 100%)';
  };
  ...
  return (
      <div className='volume'>
        <i className={!mute ? volIcon : 'fas fa-volume-mute'} 
          onClick={() =>!status && 
          setState(state => ({...state, mute: !mute}))}></i>
        <input 
          id='slider' 
          disabled={status}
          type='range' 
    ...
    <div id='display'>
      <h1>Beat Machine</h1>
        ...
        <button className='power' 
          onClick={()=>setState(state => ({...state, status: !state.status}))}>
